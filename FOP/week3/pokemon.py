...
class Pokemon:
    def __init__(self, name, level):
        self.name = name  # Use __ to prevent it from being accessed unless strictly done
        self.level = level
        self.moves = set()  # Use a set to prevent duplicates when appending
...
def battle(trainer1, trainer2):
    ...
    pokemons1 = trainer1.pokemons
    pokemons2 = trainer2.pokemons
    ...
    for i in range(rounds):
        level1 = pokemons1[i].level
        level2 = pokemons2[i].level
    ...
    if score1 >= score2:
        print(f"{trainer1.name} won the battle!")
    else:
        print(f"{trainer2.name} won the battle!")
